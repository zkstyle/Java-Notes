...
## finally
   ...
   易错点　在try-catch-finally语句中执行return语句。因为finally用法特殊，所以会撤销之前的return语句，继续执行最后的finally块中的代码。 
   
## java 为什么重写equals一定要重写hashcode？

重写equals是因为需要判断对象是否相等　因为==判断对象地址是否相等

重写equals是因为当两个对象equals相等时　可能hashcode不相等
